...
def parse(layout_text: str, is_supplementary: bool = False) -> list[Match]:
    ...
    while i < n:
        ...
        # --- item row: starts with a number (possibly N.N) in the left col
        im = re.match(r"^(\d+(?:\.\d+)?)\s+(.*)$", line)
        connected_hdr = re.match(r"^(\d+\.\d+)\s+Connected\b", line)
        if im and _col(line) <= ITEM_NUM_COL_MAX:
            item_no = im.group(1)
            rest = im.group(2)
            connected_to = None
            if "." in item_no:
...
COMMON_TOKENS = {
    ...
    "mohd", "mohammad", "mohammed", "raj", "lal", "chandra", "nath",
    ...
    "national", "insurance", "bank", "cooperative", "coop", "sahakari",
...
def norm_num(s: str) -> str:
    """Normalise any number form to compare across Diary/SLP/etc."""
    s = s.lower()
    s = re.sub(r"[^0-9/]", "", s)
    return s
...
def match_watchlist(parsed: list[Match], watchlist: list[dict]) -> list[dict]:
    ...
    for w in watchlist:
        ...
        for m in parsed:
            by_number = False
            m_diary = norm_num(m.diary_number)
            m_case = norm_num(m.case_number)
            if w_diary and w_diary == m_diary:
                by_number = True
            if w_case and (w_case == m_case
                           or w_case in m_case or m_case in ("d" + w_case)):
                by_number = True
            by_title = False
            if w_tokens:
